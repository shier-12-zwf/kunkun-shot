...
- There is no in-app auto-update or notarized public installer. v0.3.3 will attach one informed-testing DMG only after a clean rebuild passes the release checklist; its outer container will not be separately code-signed and its contained `.app` will use the fixed local development certificate. No application ZIP is planned, and updates remain manual.
