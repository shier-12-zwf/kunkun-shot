...
An open-source, macOS-first Electron screenshot utility with region capture and annotation, pinned reference windows, OCR, QR recognition, experimental scrolling capture, region recording, and optional AI-assisted image Q&A, translation, and rewriting.
...
- Pin images, text, colors, or Finder files to the desktop; pins support always-on-top, locking, mouse pass-through, thumbnails, and restoring the most recently closed item.
...
- Region recording to WebM, or conversion through FFmpeg to H.264 MP4/GIF.
- An AI workspace for OCR-assisted Q&A, translation, summarization, and rewriting; a configured vision model can receive a selected image directly.
...
- Local OCR prefers language files under the repository/application `tessdata` resources. If required data is missing, Tesseract.js may fall back to network loading, so confirm the files before relying on offline use.
...
`npm run dist:mac` is the local packaging entry point, but a successful command does not prove that the artifact is signed, notarized, or accepted by Gatekeeper on another Mac. Every binary release must state its actual signing and notarization status.
