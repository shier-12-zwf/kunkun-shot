...
- 截图编辑器的第一个一级动作是“翻译”，“OCR”紧随其后；贴图、复制和取消也在一级，低频动作收进“更多”。
- 将图片、文字、颜色或 Finder 文件贴到屏幕；图片贴图可标注、裁剪、90° 旋转、水平/垂直翻转，多张贴图可分组移动、折叠/展开与解组；还可用本地 KaTeX 生成 LaTeX 公式贴图。复制、保存、OCR、AI 和拖出使用合成结果，正常退出会恢复工作区。
...
- 截图可导出 PNG、JPEG、WebP、BMP、AVIF 或单页 PDF；截图与录屏可分别设置文件名变量模板，同名文件自动追加序号。历史页可将最多 100 张图片按选中顺序合并为多页 PDF。
- 区域录屏可保存 WebM，或通过 FFmpeg 转换为 H.264 MP4/GIF。系统声音和麦克风独立可选，还可把摄像头画中画、鼠标点击、特殊按键/修饰键快捷键及画笔标注写入成片。录制裁剪与操作提示按实际视频像素做 Retina/多屏映射。
- AI 工作台支持 OCR 后问答、翻译、总结和润色；AI 表格结果可逐单元格编辑，并复制为 CSV、TSV 或 GFM Markdown，导出时防止常见的表格公式注入。公式识别输出 LaTeX；支持视觉的模型可直接接收用户选中的截图。
- 长截图支持纵向/横向人工滚动、失败后继续、删除错帧并从原始帧重新拼接、撤销/重做、固定顶部/底部区域自动建议与手动值，以及导出裁剪。它仍为实验性能力，动态或复杂滚动内容仍需人工复核。
