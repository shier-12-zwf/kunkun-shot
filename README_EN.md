...
### v0.3.6: actual scrolling-capture fixes

Fixes stationary pages pausing at one frame, sparse text and uniform margins blocking later overlaps, and macOS menu-bar clamping shifting the guide into captured content. Proven fixed headers/footers are handled automatically without requiring Apply to continue.

Press `⌘ ⇧ A` and select the content region → click Long Screenshot immediately before Translation to reuse the selection → the first frame is captured automatically, without clicking Start → scroll down in the original page → watch the live thumbnail outside the selection → click Done, choose a save location, and have the result copied after saving succeeds. The app captures and stitches automatically; it does not scroll the page for you. Canceling or failing to save keeps the session available for retry.
...
- There is no in-app auto-update or notarized public installer. v0.3.6 will attach one informed-testing DMG only after a clean rebuild passes the release checklist; its outer container will not be separately code-signed and its contained `.app` will use the fixed local development certificate. No application ZIP is planned, and updates remain manual.
