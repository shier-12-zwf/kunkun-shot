...
- [ ] 验证复制、另存、快速保存、贴图、贴图置顶/锁定/穿透/缩略、拖出文件和恢复最近贴图。/ Verify copy, Save As, quick save, pin, pin on-top/lock/pass-through/thumbnail, drag-out, and restore-last-pin.
...
- [ ] 本地 OCR 在断网且内置中英文语言包可用时完成识别；语言包缺失时的回退行为有明确提示。/ Local OCR works offline with bundled Chinese/English data; missing-data fallback is clearly communicated.
...
- [ ] 录屏分别导出 WebM、H.264 MP4 与 GIF；验证取消、剪辑与超限保护。当前录屏不采集音频。/ Export WebM, H.264 MP4, and GIF; verify cancel, trim, and size-limit protection. Recording currently does not capture audio.
