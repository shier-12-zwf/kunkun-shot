...
> Status: early preview, v0.3.7. See the [source verification notes](docs/release-notes-v0.3.7.md) and [GitHub Release](https://github.com/shier-12-zwf/kunkun-shot/releases/tag/v0.3.7) for artifact and download-verification records. Automated tests do not imply that every hands-on flow has passed acceptance. Application binaries must be rebuilt from the release source and pass negative-content scans before upload. The GitHub Release attaches exactly one informed-testing DMG in addition to GitHub's generated source archives, and no application ZIP. The outer DMG is not separately code-signed; its contained `.app` uses the fixed local development certificate, not Apple Developer ID, and is neither notarized nor stapled. The project does not claim Windows/Linux support. See the [release checklist](docs/RELEASE_CHECKLIST.md) for its risk boundaries.

> Current source version: `v0.3.7`.

Start with the [complete Chinese user guide](docs/使用指南.md). The [PixPin comparison and improvement report](docs/PixPin-竞品对比与改进报告.md) documents the evidence, current gaps, and priorities. v0.3.7 keeps scrolling under your control, makes Copy the default long-screenshot action, and provides Save separately. Use the Long Screenshot button before Translation, then scroll the original page yourself. See section 12 of the user guide and the [v0.3.7 release notes](docs/release-notes-v0.3.7.md) for the current verification status. Earlier stitching and guide-alignment fixes are recorded in the [v0.3.6 release notes](docs/release-notes-v0.3.6.md); the pin-visibility hotfix remains documented in the [v0.3.3 release notes](docs/release-notes-v0.3.3.md). See the [changelog](CHANGELOG.md) for version history.
...
- A top-level Long Screenshot button immediately before Translation starts scrolling capture with the current selection. Translation and its target-language selector remain adjacent instead of under More, followed by OCR. Pin, Copy, and Cancel also remain top-level; lower-frequency actions live under More.
...
- Scrolling capture automatically captures the first frame after region selection, keeps a transparent, click-through selection outline visible, and previews the stitched result outside the selection while you manually scroll the original page. The tool samples and stitches; it does not scroll for you. Copy is the default action, with Save available separately. Vertical and horizontal modes support pause/continue, raw-frame deletion and restitching, undo/redo, suggested or manually configured fixed top/bottom regions, and output cropping. Failed recomposition rolls back to the last exportable result, and unreachable raw frames are released under the memory budget. It remains experimental and requires manual review on dynamic or complex pages.

### v0.3.7: manual scrolling, default Copy, separate Save

This version separates copying from saving and addresses missed final 1–2 px of scrolling and false overlap matches caused by animated foreground content against uniform backgrounds. It retains the earlier fixes for stationary pages pausing at one frame, sparse-text overlaps, and macOS guide alignment. Proven fixed headers/footers are handled automatically without requiring Apply to continue. Reliable overlap must cover at least about 10% of the content region; if overlap is insufficient or ambiguous, pause briefly or scroll back a little, then continue forward. The last good image is retained. These changes do not guarantee correct stitching on every animated page; see the [release notes](docs/release-notes-v0.3.7.md) for actual verification evidence and remaining limits.

Press `⌘ ⇧ A` and select the content region → click Long Screenshot immediately before Translation to reuse the selection → wait for the automatic first frame → put the pointer inside the selection and scroll down yourself → watch the live thumbnail and image height grow → click Copy → switch to an application that accepts pasted images and press `⌘ V`. Copy writes a native image to the system clipboard without opening a save dialog or requiring a file first.

To create a file instead, click the separate Save button and choose a location. Export follows the screenshot format settings, with PNG as the default; Save does not modify the clipboard. Either successful action closes the capture session. Canceling the save dialog, a save failure, or a copy failure pauses capture and retains the image and raw frames for retry: choose Copy, Save, or Continue without starting over. The toolbar's Cancel button instead discards the whole session; its temporary frames cannot be recovered as an editable project after closing. Successful copying creates a history copy only when automatic history is enabled; successful saving follows the existing screenshot-history rules.

The compact toolbar shows direction, Pause/Continue, Adjust, Save, Copy, and Cancel. Clicking Adjust pauses capture and expands frame editing, fixed top/bottom controls, and output cropping. Clicking Continue collapses the adjustment panel and resumes capture. Vertical is the default; switch to horizontal while only the first frame is retained, before new content has been stitched. Direction is locked once two frames are retained.

Prefer clicking the buttons. While the long-screenshot toolbar has focus, `Enter` or `⌘ C` copies, `⌘ S` saves, and `Escape` cancels the session. These are not global shortcuts: when focus remains in the original page, that application handles the keys. Editing an input or using a dropdown does not trigger export with `Enter`, `⌘ C`, or `⌘ S`. A button focused with `Tab` keeps its own Enter action—for example, Enter on Save saves; Enter on Cancel cancels. Only Enter outside input fields and buttons defaults to Copy.
...
- There is no in-app auto-update or notarized public installer. Artifact and download-verification records belong in the v0.3.7 GitHub Release. Only a freshly rebuilt, verified informed-testing DMG may be attached; its outer container is not separately code-signed and its contained `.app` uses the fixed local development certificate. No application ZIP is provided, and updates remain manual.
