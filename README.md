...
- 长截图支持纵向/横向人工滚动、失败后继续、删除错帧并从原始帧重新拼接、撤销/重做、固定顶部/底部区域自动建议与手动值，以及导出裁剪。控制条会被限制在当前显示器可见区域；重拼失败会回滚到上一份可导出结果，失去引用的原始帧会释放以守住内存预算。它仍为实验性能力，动态或复杂滚动内容仍需人工复核。
...
- 长截图仍需用户人工滚动，也仍是实验性实现；动画、持续刷新、反向滚动或复杂嵌套滚动容器可能无法可靠拼接。固定顶部/底部已有辅助处理，但仍需人工确认。
